...
public class ItemSpawner : MonoBehaviour {
    ...
    private Vector3 GetRandomPointOnNavMesh(Vector3 center, float distance) {
        ...
        Vector3 randomPos = Random.insideUnitSphere * distance + center; // 중점이 center, distance

        // 내비메시 샘플링의 결과 정보를 저장하는 변수
        NavMeshHit hit;

        // maxDistance 반경 안에서, randomPos에 가장 가까운 내비메시 위의 한 점을 찾음
        NavMesh.SamplePosition(randomPos, out hit, distance, NavMesh.AllAreas); // 고정하고싶은 좌표, distance가 거리 범위, hit에 담김
